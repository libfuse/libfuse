...
static void timeout_lookup(fuse_req_t req, fuse_ino_t parent,
			   const char *name) {
...
static void timeout_open(fuse_req_t req, fuse_ino_t ino,
...
int main(int argc, char *argv[])
{
    ...
    /* Set daemon timeout */
    assert(snprintf(buf, sizeof(buf), "-odaemon_timeout=%u", daemon_timeout) == strlen(buf));
