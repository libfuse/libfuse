...
#ifndef PASSTHROUGH_HP_COMMON_HPP_INCLUDED
#define PASSTHROUGH_HP_COMMON_HPP_INCLUDED
...
static cxxopts::ParseResult parse_wrapper(cxxopts::Options& parser, int& argc, char**& argv, void (*print_usage)(char *)) {
    try {
...
static cxxopts::ParseResult common_parse_options(int argc, char **argv, void (*print_usage)(char *)) {
    ...
    auto options = parse_wrapper(opt_parser, argc, argv, print_usage);
...
static struct fuse_session* init_passthrough_fs(fuse_args *args) {
    fs.root.fd = -1;
...
#endif //PASSTHROUGH_HP_COMMON_HPP_INCLUDED
