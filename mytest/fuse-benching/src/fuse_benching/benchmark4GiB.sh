...
#	1MiB.bin, 2MiB.bin, 4MiB.bin, 16MiB.bin, 64MiB.bin, 256MiB.bin, 512MiB.bin, 1GiB.bin
#
# Usage:
#   ./benchmark4GiB.sh
#
...
# The script should just test with the /sizedFiles/4GiB.bin file.
...
TESTED_SIZED_FILE="/home/kvelusamy/Desktop/fuse-benching/sizedFiles/4GiB.bin"
...
RESULTS_CSV="results4GiB.csv"
echo "max_read,iteration,bandwidth_MBps,elapsed_seconds" > "$RESULTS_CSV"

ls -lah /usr/local/lib/x86_64-linux-gnu/libfuse3.so.4
...
for maxr in "${MAX_READ_SIZES[@]}"; do
  ...
  for i in $(seq 1 "$ITERATIONS"); do
    ...
    # Print iteration result and elapsed time
    echo "Iteration $i: $MBPS MB/s    Elapsed: ${ELAPSED}s"

    # Record results including elapsed time
    echo "$maxr,$i,$MBPS,$ELAPSED" >> "$RESULTS_CSV"
...
python plot_bench_4GiB.py "$RESULTS_CSV"
