...
{
	uint64_t hash = ((uint32_t) ino * 2654435761U) & (f->id_table.size - 1);
	uint64_t oldhash = hash & (f->id_table.size / 2 - 1);
			...
			const char *name)
{
	...
	hash &= (f->name_table.size - 1);
	oldhash = hash & (f->name_table.size / 2 - 1);
