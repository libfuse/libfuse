...
static void *fuse_sync_init_worker(void *data)
{
	 ...
	 * returns.
	 */
	while (!atomic_load(&se->mount_finished)) {
...
int fuse_session_mount(struct fuse_session *se, const char *_mountpoint)
{
	...
	/*
	 * Try synchronous FUSE_INIT for privileged (direct) mounts.
	 * Only if early daemonize is active and io-uring is disabled.
	 */
	if (se->debug)
		fuse_log(FUSE_LOG_DEBUG,
			 "fuse: want_sync_init: %d, daemonize_active: %d\n",
			 se->want_sync_init, fuse_daemonize_active());
	if (se->want_sync_init || fuse_daemonize_active()) {
		...
		fuse_log(FUSE_LOG_DEBUG, "fuse: not trying sync init\n");
		fd = fuse_kern_mount(mountpoint, se->mo);
		if (fd == -1)
			goto error_out;
	}
